...
class rdt:
    ...
    def accept(self):
        try:
            ...
            logging.info("receive "+ receive.decode()+" from "+str(a1))
    ...
    # 这下面的函数都需要保证connected标志为True.
    # TODO: 这里需要考虑一下seq序列号的从0xff->0x0的过程
    #       不过这会在传输100bytes*256时刻出现问题,对于version 0.1不考虑这个问题.
    ...
    def gbn_send(self,info:str):
        with self.Lock:
            ...
            for i in range(max_group_number):
                ...
                if i == max_group_number-1:
                    secondchunk = self.constructSecond(0,len(data)-self.group_size*i)
                    datachunk = data[self.group_size*i:] # TODO: padding add
                else:
